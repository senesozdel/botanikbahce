...
import { updatePlant, fetchPlants, setCurrentPlant } from '../../features/plants/plantsSlice';
...
const EditPlantPage = () => {
  ...
  const { plants, currentPlant, status } = useSelector(state => state.plants);
  ...
  useEffect(() => {
    ...
    if (!currentPlant && plants.length > 0) {
    ...
    if (currentPlant) {
      ...
      setAccessionnumber(currentPlant.accessionNumber || '');
      setMaterialType(currentPlant.materialType || '');
      setDescription(currentPlant.description || '');
      setOrigin(currentPlant.origin || '');
      setLocation(currentPlant.location || '');
      setCoordinates(currentPlant.coordinates || '');
      setCollectionDate(currentPlant.collectionDate || '');
      setCategoryId(currentPlant.categoryId?.toString() || '');
      setCollectorId(currentPlant.collectorId?.toString() || '');
      setLoading(false);
    }
  }, [dispatch, id, plants, currentPlant, status]);
  
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    
    if (!name.trim() || !accessionNumber.trim()) {
    ...
    try {
      await dispatch(updatePlant(plantData)).unwrap();
      navigate('/plants');
    } catch (err) {
      setError(err || 'Bitki güncellenirken bir hata oluştu');
